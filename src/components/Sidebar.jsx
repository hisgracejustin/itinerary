    ...
    <aside className="w-full h-full overflow-y-auto shrink-0 bg-white border-r border-outline/40 flex flex-col pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)]">
      ...
      <nav className="p-3 pt-4">
        ...
        <NavItem
          ...
          icon={
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M7 3h10a2 2 0 012 2v16l-3-2-4 2-4-2-3 2V5a2 2 0 012-2zm2 5h6m-6 4h6" />
            </svg>
          }
          label="Expenses"
        />
        ...
        {/* Hard navigation (plain <a>): /sheet lives outside the app shell, and
            ...
            copy when there's no connection. Offline, the click handler bypasses
            navigation entirely (iOS won't route a fresh worker's first-launch
            navigations) and swaps the cached sheet in from the Cache API. */}
        {/* No onNavigate here: the handler always navigates (or swaps the
            document), and a drawer-close state update racing the departure is
            exactly the kind of churn the handler exists to avoid. */}
        <NavItem
          to="/sheet"
          hard
          onClick={handleOfflineSheetClick}
          icon={
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
